...
namespace Appcompras.Datos
{
    public class Ddetallecompras
    {
        ...
        public async Task<List<Mdetallecompras>> MostrarTotales()
        {
            return (await Cconexion.firebase
                .Child("Detallecompra")
                .OnceAsync<Mdetallecompras>()
        ...
        public async Task<string> SumarTotales()
        {
            var funcion = new Ddetallecompras();
            var lista = await funcion.MostrarTotales();
            double total = 0;
            foreach (var hobit in lista)
            {
                total += Convert.ToDouble(hobit.Total);
            }
